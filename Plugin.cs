...
namespace WaterGunLib
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        //internal class StartOfRound_start
        //{
        //    [HarmonyPostfix]
        //    public static void TestItemSpawner(StartOfRound __instance)
        //    {

        //        GameObject positionToSpawn = GameObject.Find("StartGameLever");
        //        var itemspawner = positionToSpawn.AddComponent<ItemSpawner>();
        //        itemspawner.possibleItemNames = new string[]
        //        {
        //            "Detonator"
        //        };
        //        GameObject spawnedItem = itemspawner.SpawnRandomItem(positionToSpawn.transform.position + Vector3.up * 1f);
        //        Debug.Log(spawnedItem.name + "Spawned xd");
        //    }
        //}
        ...
        internal class Terminal_Awake
        {
            ...
            public static void Prefix(Terminal __instance)
            {
                ...
                foreach (var pair in registeredItems)
                {
                    ...
                    if (StartOfRound.Instance.allItemsList.itemsList.Any((Item x) => x.itemName == key.item.itemName))
                    {
                        Debug.LogError("[WaterGunLib]: " + key.item.itemName + "Already exists in itemsList");
                        continue;
                    }

                    StartOfRound.Instance.allItemsList.itemsList.Add(key.item);
                    ...
                    else if (!key.item.isScrap)
                    {
                        if (__instance.buyableItemsList.Any(item => item == key.item)) continue;
                ...
                {
                    if (item.Key.item.isScrap) continue;
